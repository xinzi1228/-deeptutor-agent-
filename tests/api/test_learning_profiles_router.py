...
def _client(tmp_path, monkeypatch) -> TestClient:
    user = CurrentUser(id="u_test", username="tester", role="user", scope=UserScope(kind="user", user_id="u_test", root=tmp_path))
...
def test_profile_create_unlock_active_and_lock(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch) as client:
        ...
        unlocked = client.post(f"/api/v1/learning-profiles/{profile_id}/unlock", json={"pin": "1234"})
...
def test_cross_account_profile_is_not_visible(tmp_path, monkeypatch):
    ...
    with _client(tmp_path, monkeypatch) as client:
        response = client.post(f"/api/v1/learning-profiles/{foreign.id}/unlock", json={"pin": "1234"})
    assert response.status_code == 404


def test_wrong_pin_does_not_set_grant_cookie(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch) as client:
        profile_id = client.post("/api/v1/learning-profiles", json={"name": "小明", "pin": "1234"}).json()["id"]
        response = client.post(f"/api/v1/learning-profiles/{profile_id}/unlock", json={"pin": "9999"})
