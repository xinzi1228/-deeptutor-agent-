...
class LearningWorkspaceService:
    ...
    def require_access(
        self, resource: Resource, action: Literal["read", "write"], *, actor: str = "coach"
    ) -> None:
        allowed = (
            _READABLE if action == "read" else (_COACH_WRITABLE if actor == "coach" else _READABLE)
        )
    ...
    def rebuild(self, *, rebuild_course: bool = False, confirmed: bool = False) -> dict[str, Any]:
        ...
        from deeptutor.services.knowledge_graph import (
            KnowledgeGraphStore,
            _load_bank,
            _load_competency_tree,
        )
        ...
        manifest = {
            "schema_version": 1,
            "last_rebuild": {"at": datetime.now(tz=timezone.utc).isoformat(), "result": result},
            "assets": self.asset_versions(),
        }
    ...
    def add_inbox(
        self, raw_text: str, *, source: str, context: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        ...
        if any(item.get("raw_text") == text and item.get("source") == source for item in recent):
            return {
                **next(
                    item
                    for item in recent
                    if item.get("raw_text") == text and item.get("source") == source
                ),
                "deduplicated": True,
            }
        item = {
            ...
            "suggested_type": None,
            "resolved_to": [],
            "audit": {"actor": "coach", "operation": "create"},
        }
        self.root.mkdir(parents=True, exist_ok=True)
        with self.inbox_file.open("a", encoding="utf-8") as handle:
    ...
    def views(self) -> dict[str, Any]:
        ...
        for record in reversed(records):
            ...
            if pattern and record.get("pattern_status") == "confirmed" and pattern not in errors:
        ...
        return {
            ...
            "relations": {
                "inbox_to_task": [
                    {"inbox_id": i["id"], "task_id": i.get("context", {}).get("task_id")}
                    for i in self.list_inbox()
                    if i.get("context", {}).get("task_id")
                ]
            },
        }
    ...
    def organize_inbox(self, item_id: str, *, resolved_to: list[str]) -> dict[str, Any]:
        ...
        for item in rows:
            if item.get("id") == item_id:
                ...
                item["resolved_to"] = resolved_to
                item["organized_at"] = datetime.now(tz=timezone.utc).isoformat()
                found = True
                after = dict(item)
        if not found:
            raise ValueError("未找到待整理问题")
        from deeptutor.services.authorization.policy import authorize_profile_operation
        ...
        authorize_profile_operation(
            ...
            resource_type="inbox_item",
            resource_id=item_id,
            request_id=f"inbox-organize:{item_id}:{after.get('organized_at') if after else ''}",
            before=before,
            after=after,
        )
