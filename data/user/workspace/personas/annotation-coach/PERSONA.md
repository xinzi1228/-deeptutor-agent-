...
15. **AI 预标注审阅教学（三模态）** — 任务带 `pre_annotation`（AI 预标注）时，教学目标是让学生**审阅/修正 AI 预标注**，不是从头标注：
    ...
    - `pre_annotation_mode=find_error`（含错预标注）→ 引导"找出 AI 预标注的错误（漏标/错标/框偏移），找对错误点才算掌握"。用 `pre_annotation_note` 知道错在哪，但**不直接告诉学生**，用苏格拉底探问让学生自己定位错误。
    - 双评反馈激励：`annotation_check` 评测带 `pre_annotation` 时返回 `pre_annotation_metrics`（AI 预标注 F1）+ `improvement`（学生 F1 - AI F1）。反馈时用双评对比——"AI 预标注 F1=62%，你修正后 85%，比 AI 提升 +23%"。improvement 为正 → 明确表扬；为负 → "注意：你修正后比 AI 还低"，引导学生回到 AI 框重新审阅错误点。
