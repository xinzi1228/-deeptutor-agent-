...
export default function UnifiedAnnotationWorkbench({
  ...
  const [result, setResult] = useState<{ metrics: Record<string, unknown>; report: string; formal: boolean; scoreRecord?: AnnotationScoreRecord | null } | null>(null);
  ...
  const submit = useCallback(async () => {
    ...
    try {
      ...
      setSaveState("synced");
      setResult({ metrics: data.grade?.metrics || data.attempt?.metrics || {}, report: data.grade?.report || data.attempt?.report || "提交成功", formal: true, scoreRecord: data.score_record });
  ...
  useEffect(() => {
    ...
    const retry = () => {
      void retryPendingAnnotationRevisions().then((data) => {
        const completedRow = data.completed.find((item) => item.attempt?.task_id === task.id);
        const completed = completedRow?.attempt;
        if (!completed) return;
        setSyncPending(false);
        setSaveState("synced");
        setResult({ metrics: completed.metrics || {}, report: completed.report || "正式修订已同步", formal: true, scoreRecord: completedRow?.score_record });
        if (activeProfileId) {
...
function BBoxEditor({ task, predictions, onChange, onImageSizeChange, onInteractionState }: { task: AnnotationTask; predictions: Array<Record<string, unknown>>; onChange: (value: Array<Record<string, unknown>>) => void; onImageSizeChange: (size: ImageBounds) => void; onInteractionState?: (state: Record<string, unknown>) => void }) {
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      const typing = target?.tagName === "INPUT" || target?.tagName === "TEXTAREA" || target?.tagName === "SELECT";
      if (typing) return;
      const handled = (run: () => void) => { run(); event.preventDefault(); event.stopImmediatePropagation(); };
      if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "z") { handled(event.shiftKey ? handleRedo : handleUndo); return; }
      if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "y") { handled(handleRedo); return; }
      if (!event.ctrlKey && !event.metaKey && !event.altKey) {
        const key = event.key.toLowerCase();
        if (key === "v") { handled(() => setTool("select")); return; }
        if (key === "b" || key === "r") { handled(() => setTool("draw")); return; }
      ...
      if (event.shiftKey && !event.ctrlKey && !event.metaKey && event.code === "Digit2") { handled(() => setZoom(1)); return; }
      if (/^[1-9]$/.test(event.key) && !event.ctrlKey && !event.metaKey && !event.shiftKey) {
  ...
  useEffect(() => {
    onInteractionState?.({
      ...
      selected_object_id: state.selectedIds[0] || "",
