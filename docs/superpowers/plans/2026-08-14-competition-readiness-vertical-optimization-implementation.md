...
当前进度：阶段 0 的工程基础、阶段 1、阶段 2、任务 3.1 至 3.5 和任务 4.1 已有提交；任务 0.4 仍需要人工专业终审。下一项从任务 4.2 开始，详细状态以 AI 执行交接书和当前 Git 历史为准。
...
实施结果（2026-08-15）：已由提交 `40ddb440` 完成。统一策略覆盖学生、教师只读、教师代管和伙伴合成账号；代管票据必须包含原因与明确 scope，闲置与绝对有效期均按 30 分钟约束。标注、学习记录、记忆、当前任务、收集箱和可视化作品在 Store 边界执行权限；聊天和统一 WebSocket 继续使用同一策略。代管仅开放任务分配、收集箱整理、教师反馈和已审核派生记录修正，原始对话、标注、初次成绩和 PIN 不可修改。逐写审计只保存请求、资源、操作与前后内容 SHA-256，不保存学生正文。

专项验收：角色/审计/存储边界/隔离/伙伴记忆共 62 项通过；学习档案 API 5 项通过。后端扩大回归在 Windows 环境得到 3669 项通过、12 项跳过；本次引入的 3 项伙伴记忆失败已修复并单独回归通过。其余 33 项为既有环境或基线问题，包括缺少 Telegram/Slack 可选依赖、POSIX 专属 `resource`/路径断言、GBK 默认解码和旧测试数据污染，不能记作本任务通过，也不阻塞权限专项验收。
