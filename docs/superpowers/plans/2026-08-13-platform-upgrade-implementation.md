...
## 13. 二次深度收口（2026-08-13）

在首轮“已完成主路径”之后，又按代码审计结果补齐了以下原先只完成一部分的功能：

1. 教师只读与代管审计扩大到会话、记忆、学习档案工作区、题本、报告、插件和 WebSocket/Agent 工具写入，不再只覆盖标注与 Label Studio 两条路。
2. 四个原生 HTML 教学标注入口的主产品路径迁移为统一 React 标注台，覆盖图片、文本、音频、视频和 9 类任务；服务端档案草稿、恢复、撤销/重做、快捷键、提交和评分成为主链路，旧 HTML 只保留兼容用途。
3. 专业模式只展示当前档案被分配的任务；网关向工作台注入最小事件桥接，教练可知道框数量、保存、撤销、标签和切题，但不采集鼠标轨迹或未保存坐标。
4. 标注教练的可视化不再在 `done` 事件消失；学习报告也消费统一作品协议。作品卡片现支持全屏、同数据换图、JSON/PNG 下载、保存、加入学习资料和删除。
5. imagegen 支持管理员目录内模型的单次选择，不修改全局默认；生成图片进入 `generated_image` 作品协议并记录模型、提示词、来源和会话。
6. 数字图表新增 `read_learning_chart_data` 可信快照工具。`create_visualization` 必须核对 `dataset_ref` 及 labels/datasets，任何模型改数或补数都会被服务端拒绝。
7. 三名可视化专家加入统一专家目录；图表专家只能先读可信学习数据再建图，插画专家仍只设计提示词。
8. 六张透明教练状态资产已经生成并接入状态机：默认、思考、鼓励、提醒、成功、错误。
9. 学生不能再创建/导入/删除任意 Skill，也不能直接执行插件工具；这些高级操作改为管理员路径。白名单市场中的“报告卡片增强”已经从占位条目变为真实报告展示能力。
10. 能力中心区分“资料文件存在”和“可检索索引 ready”，并明确显示 Embedding 缺失；新增 `/annotation`、`/progress`、`/capabilities` 路由包体预算。

二次收口提交范围为 `51d9963f` 至 `1461b4d9`。最终验证结果：
