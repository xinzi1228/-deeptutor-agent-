...
状态：任务 4.2（`98b2d2ca`）、4.3（`163909f6`）、4.4 代码（`03dd8ef0`）、5.1（`fa827993`）、5.2（`89f6d928`）、5.3（`0f5b48d4`）均已完成，详情见本文件末尾交付记录。任务 4.4 的真实用户执行、5.1/5.2 的目标电脑性能测量与三次演示属人工验收，尚未进行。
...
## 18. 任务 5.3 交付记录（2026-08-15）

```text
任务：5.3 同步说明文档和 AGENTS.md
对应规格：docs/superpowers/specs/2026-08-14-release-readiness-gates-design.md §8
实现结果：
  - AGENTS.md 同步当前状态：学生四入口、管理员五中心工作台、教师工作台、
    可恢复初始化向导状态机与扩展白名单、真实用户证据服务；标注专业模式只
    经同源网关进入，删除共享管理员账号/直接 iframe 描述
  - AGENTS.md 记录最终测试命令与基线（pytest 3723/12/33、node 334、E2E 11
    项、perf:check 全过），并明确仍属人工验收/外部条件的 0.4 内容终审、
    4.4 真实用户、5.1/5.2 竞赛电脑测量、真实模型密钥与 Embedding 验收
  - 更新关键模块表：能力中心/初始化、五中心、教师台、扩展市场、用户测试、
    性能预算
  - demo-script.md / cannot-demo.md：删除 admin@localhost 共享账号指令，
    改为经同源网关进入专业模式；页面名对齐四入口；测试基线更新为最新数字
  - README.md：能力中心入口描述对齐管理员工作台
  - label-studio 能力报告：补充“已废弃直接 iframe/共享账号描述”说明
修改文件：AGENTS.md、README.md、docs/{demo-script,cannot-demo,
  label-studio-1.23-capability-report}.md
数据迁移：无。
测试命令与结果：文档变更，无自动化测试；git diff --check 通过。
人工验收：需在目标电脑复核文档描述与真实产品一致。
外部条件/未验证项：真实模型/Embedding 在场后的最终数字仍待更新。
安全与隔离检查：未删除共享账号之外的用户材料；未暂存用户未跟踪文件。
提交号：0f5b48d4
下一任务是否满足前置条件：全部代码任务已交付；剩余为人工验收与外部配置门槛。
```
