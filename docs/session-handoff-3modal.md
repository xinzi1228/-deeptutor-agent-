...
> **第二轮优化（2026-08-06）**：设计见 `docs/superpowers/specs/2026-08-06-optimize-round2-design.md`（O1-O11 借鉴 Bloom/Agent_Memory_Techniques）。P0 已实施：O1 记忆路由 fallback（read_bucket 当前区空回退全局）+ O5 delegate 专家移除 write_learning_record（学习记录只由总控落盘）。P1 待做：O10 渐进加载 / O7 syllabus 进度联动 / O3 遗忘标记。
> 当前两条线：
