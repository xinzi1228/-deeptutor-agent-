...
> ⑦ 第八轮（stablyai/orca 前端，`2026-08-08-orca-frontend-design.md` O1-O3）：**设计 + 实现计划已 commit（`plans/2026-08-08-orca-frontend.md`），实施未开始**——O1 共享状态点原语 + 状态词汇表（AgentStateDot：working 黄 spin/done emerald check/waiting 琥珀问号/blocked 红点/idle 灰点 + a11y label，Coach 状态环 H2 对齐词汇表）+ O2 时长反馈规范（0-100ms 无反馈/100ms-1s 禁用/1-3s 禁用+spinner/3s+ 阶段标签 + 预占空间 + 延迟加载）+ O3 单色设计纪律（color-mix 不造 hex）。另：Orca 专家 Fleet 看板（Needs You/Working/Done/Idle + unseen ack + View Transition）为可选项。参考源码 clone：`%TEMP%\opencode\refs\orca\orca\`。**待实施清单**：仅此 1 份设计（O1-O3）+ round2 的 6 项 P1/P2（O2/O4/O6/O8/O9/O11）。
> 当前两条线：
