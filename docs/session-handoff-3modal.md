...
> ② **三模态标注拓展**：`docs/3modal-annotation-plan.md` + `research.md`；基础三模态已实施（`91f494c3`：text/audio/video 标注台 + annotation_check 5 新 task_type + tIoU/WER/segment F1 + task_bank 22 任务 + competency 视频组 + text/video-guide 规范）。**AI 预标注审阅教学完成**（2026-08-10，8 commit，借鉴 doccano-mini few-shot + labelme AI 框 UX）：grilling 访谈定案（UX 优先）+ GitHub 调研（doccano-mini `%TEMP%\opencode\refs\doccano-mini\` + labelme `%TEMP%\opencode\refs\labelme\`）。功能：task_bank 3 任务加 `pre_annotation`（task1 review 正确预标注 / task2+task4 find_error 漏标+框偏移，含 mode+note）+ `get_annotation_task` 透传 + `annotation_check` 双评（pre_annotation_metrics + improvement，仅 bbox）+ HTTP /check 透传 + `annotation_tool.html` AI 框 UX（虚线琥珀 #d97706 + AI 角标 + 全部接受/清除 + 全框可编辑）+ ChatMessages 写 `annotation_pre_annotation` localStorage（关键：get_annotation_task 结果→标注台）+ home 双评反馈消息 + PERSONA 核心原则 #15。spec review 抓 CRITICAL（localStorage 键无人写入→ChatMessages 补）+ MEDIUM（畸形预标注 500→try/except）全修。**27 测试过 + tsc clean + 端到端冒烟过**（横幅/AI 框/双评 AI 0.75→学生 1.00）。计划：`plans/2026-08-10-ai-preannotation.md`。**可扩展**：text/audio/video 标注台预标注（当前仅图像 bbox）、LLM 动态生成预标注（doccano-mini few-shot，当前预置）、视觉对比卡（当前文字）。
