...
> 当前两条线：
> ① 7 议题：⑤⑥③ 彻底完成；①② Phase 1-3 全完成；④ Phase 1-2 + 3a + **3b + 3c 完成**；⑦ Phase 1-2 完成。剩余：④ 3b 前端反馈接入（待 iframe 归属确认，Coach 气泡展示为无侵入过渡）；竞赛交付材料（9/1）。规划见 `plans/2026-08-05-ls-annotations-workbench.md` + `plans/2026-08-05-ls-annotations-workbench-3bc.md`（3b 评分端点 `POST /api/v1/annotation/check` 薄封装 `annotation_check._*_dict` 10 种 task_type + `AnnotationProgress.tsx` 能力树进度条，均已双 review 完成，28 测试通过）。
