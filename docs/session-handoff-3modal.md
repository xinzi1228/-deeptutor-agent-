...
> ⑦ 第八轮（stablyai/orca，`2026-08-08-orca-frontend-design.md` O1-O3 前端 + `2026-08-08-orca-fleet-design.md` F1-F3 功能）：**两份设计 + 前端实现计划已 commit（`plans/2026-08-08-orca-frontend.md`），实施未开始**——前端 O1 共享状态点原语 + 状态词汇表（AgentStateDot：working 黄 spin/done emerald check/waiting 琥珀问号/blocked 红点/idle 灰点 + a11y label，Coach 状态环 H2 对齐词汇表）+ O2 时长反馈规范（0-100ms 无反馈/100ms-1s 禁用/1-3s 禁用+spinner/3s+ 阶段标签 + 预占空间 + 延迟加载）+ O3 单色设计纪律（color-mix 不造 hex）；功能 F1 专家 Fleet 看板（Needs You/Working/Done/Idle + unseen ack + askSummary 横幅，消息流内专家任务卡，纯前端零后端）+ F2 WS 即推送 + seq/epoch 重放（任务完成通知，需后端事件）+ F3 Smart attention 排序（Coach 审阅优先级）。参考源码 clone：`%TEMP%\opencode\refs\orca\orca\`。**待实施清单**：orca 前端 O1-O3（计划已就绪）+ orca 功能 F1-F3（仅设计）+ round2 的 6 项 P1/P2（O2/O4/O6/O8/O9/O11）。
> 当前两条线：
