...
> ② **三模态标注拓展**：`docs/3modal-annotation-plan.md` + `research.md`；基础三模态已实施（`91f494c3`）。**AI 预标注审阅教学完成**（见⑦第八轮 after F2）。**演示准备完成（2026-08-10，明天向指导老师展示）**：亲读全部 26 份设计文档（14 superpowers/specs + 12 docs/specs）→ `docs/demo-plan-design.md`（演示方案：会话预跑+页面穿插，方案 A）→ 用 WS 脚本 `%TEMP%\opencode\demo_chat.py`（AUTH 关闭，`ws://127.0.0.1:8001/api/v1/ws` 发 `{"type":"message","content":...}`）跑通 **9 条演示会话**（①完整教学闭环 ②专家委派 ③能力评估 ④知识库溯源 ⑤AI预标注审阅 ⑥陪伴教学 ⑦能力图谱 ⑧理论教学IOU ⑨交互练习卡quiz_card），已重命名（侧边栏可见，前端验证点击加载+渲染通过：工具调用按钮/任务卡/评分卡/双评/苏格拉底/卡点介入全展示）→ 演示脚本 `docs/demo-script.md`（三幕：闭环/亮点/平台）+ 不可展示清单 `docs/cannot-demo.md`（LS 8080未开/语音未配置/定时等时间/真人操作，大白话+替代方案）。**演示会话数据在 `data/user/chat_history.db`（gitignored，clone 需重跑）**。工具：`python demo_chat.py "消息" [--session id] [--rename id title] [--list]`；重命名端点 `PATCH /api/v1/sessions/{id}`；列表 `GET /api/v1/sessions?limit=N`（`/chat/sessions` 是 legacy 返回空，勿用）。
