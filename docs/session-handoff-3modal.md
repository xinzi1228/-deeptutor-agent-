...
> ⑦ 第八轮（stablyai/orca，`2026-08-08-orca-frontend-design.md` O1-O3 前端 + `2026-08-08-orca-fleet-design.md` F1-F3 功能）：**前端 O1-O3 已实施完成**（4 commit：`AgentStateDot` 共享状态点原语 `web/components/common/AgentStateDot.tsx`（AgentDotState 6 态 + agentStateLabel a11y，working 黄 spin/done emerald check/waiting 琥珀问号/blocked·failed 红点/idle 灰点，无 cn 工具用模板字符串）+ Coach 状态环 `STATUS_RING` 对齐词汇表（working=amber/waiting-input=muted/flash=emerald）+ ChatComposer 取消按钮 sr-only 阶段标签 + 纪律文档 `docs/frontend-design-discipline.md`（O3 单色纪律，已 -f 进 git）；tsc 全过。**功能 F1-F3 仅设计待实施**（专家 Fleet 看板 Needs You/Working/Done/Idle + unseen ack + askSummary 横幅，纯前端零后端；WS 即推送 + seq/epoch 重放，需后端事件；Smart attention 排序）。参考源码 clone：`%TEMP%\opencode\refs\orca\orca\`。**待实施清单**：orca 功能 F1-F3（仅设计）+ round2 的 6 项 P1/P2（O2/O4/O6/O8/O9/O11）。
> 当前两条线：
